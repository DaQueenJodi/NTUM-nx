...
wkick = 8
ang=gunangle-80;
repeat(9+irandom(loop))
{
with instance_create(x,y,EnemyBullet1)
{
motion_add(other.ang,2+random(2))
image_angle = direction
team = other.team
}
ang+=160/(9+(irandom(loop)))
}
}
else if random(5)<1
{
    ...
    wkick = 8
    ang=gunangle-60;
    repeat(5+(irandom(loop)*2))
    {
    
    with instance_create(x,y,EnemyBouncerBullet)
    {
    motion_add(other.ang,2+random(3))
    image_angle = direction
    team = other.team
    }
    ang+=120/(5+(irandom(loop)*2))
    }
    alarm[4]=6;
...
else if ((random(10) < 1 && !instance_exists(IDPDSpawn)) || (random(20) < 1))
{
