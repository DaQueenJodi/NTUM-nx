function scrDecideWep(argument0, cursed = 0) {
	if instance_exists(Player) {
		if UberCont.opt_gamemode=2 {
			...
			if cursed {
				//DO UNTIL wep is at or above current area, or until above 20
				do wep = round(random(maxwep-1)+1)
				until (wep_area[wep] > Player.hard+argument0 or Player.hard > 20) and wep_area[wep] <= Player.hard+argument0+5 and wep_area[wep] and ((wep != Player.wep and wep != Player.bwep) or Player.race = 7)
			} else {
				if Player.ultra_got[29]==1 {//DO UNTILL wep is above area 5 (3-1) REFINED TASTE
					do wep = round(random(maxwep-1)+1)
