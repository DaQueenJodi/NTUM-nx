...
if race = 25 && alarm[3]<1 //Mutation smith
{
    ...
    var raddrop=10;//An ally drops 5 rads
...
with instance_create(x,y,ArmourStrike)
{image_angle = point_direction(x,y,mouse_x,mouse_y)-40
Originalangle=image_angle;
team = other.team
ammo = 60;
event_perform(ev_alarm,0)
...
if race = 18//ANGEL
{
    ...
    ammo[wep_type[wep]] += (typ_ammo[wep_type[wep]]*1+skill_got[5])+extra
    ...
    dir.mytext = "+"+string( (typ_ammo[wep_type[wep]]*1+skill_got[5])+extra )+" "+string(typ_name[wep_type[wep]])
    if ammo[wep_type[wep]] = typ_amax[wep_type[wep]]
    ...
    else if ( ammo[wep_type[wep]]-round(typ_ammo[wep_type[wep]]*2.5)-skill_got[5]>=0 && my_health<maxhealth )
    {
    ammo[wep_type[wep]]-=round(typ_ammo[wep_type[wep]]*2.5)-skill_got[5];
    
    dir = instance_create(x,y,PopupText)
    dir.mytext = "-"+string(round(other.typ_ammo[wep_type[other.wep]]*2.5)-skill_got[5])+" "+string(other.typ_name[wep_type[other.wep]])
...
//REBEL
if race = 10 and my_health > 2 || (race = 10 && !(instance_exists(Ally)) && my_health > 1)
{canrebel = 1
...
if ultra_got[13]{
with enemy{
if maxhealth<=5 and x > view_xview and x < view_xview+view_wview and y > view_yview and y < view_yview+view_hview
...
if ammo[wep_type[wep]] >= wep_cost[wep]*(2+Player.skill_got[5]*2) and rad>=wep_rad[bwep]*(2+Player.skill_got[5]*2)// Check ammo of our secondary wep
{
    ...
    if (1.7>random(wep_cost[wep]) )//the random value less likely when the other weapons cost is a lot
    {
    repeat(2+Player.skill_got[5]*2)
        {
        scrFire()
...
{
mask_index=mskFloorExplo;
var xx;
var yy;
xx=16*(mouse_x div 16);
yy=16*(mouse_y div 16);
if point_distance(x,y,mouse_x,mouse_y)>16{
    if place_meeting(xx,yy,Floor)&&place_free(xx,yy)&&!place_meeting(xx,yy,enemy)&&!place_meeting(xx,yy,prop)&&!place_meeting(xx,yy,Sheep)&&!place_meeting(xx,yy,ExplosiveSheep)
    {
